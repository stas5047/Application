...
import { CalendarDays, CalendarRange, LogOut, Menu, Sparkles, User, X } from 'lucide-react';
    ...
    <nav className="fixed top-0 left-0 right-0 z-50 border-b bg-background">
        ...
        <div className="flex items-center gap-6">
          ...
          {isAuthenticated && (
            <div className="hidden sm:flex items-center gap-4">
              ...
              {/* TODO: move after AI Assistant link once Phase 22 is implemented */}
              <NavLink to="/recommendations" className={navLinkClass}>
                {({ isActive }) => (
                  <span className="flex items-center gap-1.5">
                    <Sparkles className={cn('h-4 w-4', isActive ? 'text-primary' : 'text-muted-foreground')} />
                    Recommendations
                  </span>
                )}
              </NavLink>
      ...
      {isAuthenticated && menuOpen && (
        <div className="sm:hidden border-t bg-background px-4 py-3 flex flex-col gap-3">
          ...
          {/* TODO: move after AI Assistant link once Phase 22 is implemented */}
          <NavLink to="/recommendations" className={navLinkClass} onClick={closeMenu}>
            {({ isActive }) => (
              <span className="flex items-center gap-2">
                <Sparkles className={cn('h-4 w-4', isActive ? 'text-primary' : 'text-muted-foreground')} />
                Recommendations
